...
Process runner должен зарегистрировать `onFailure` до запуска singleton-воркера. При потере leader lock воркер
сам останавливается, а listener должен сообщить об ошибке и установить ненулевой exit code, чтобы менеджер процессов
перезапустил процесс.
...
Детали failed blocks доступны только в JSON snapshot. Prometheus показывает счетчик failed jobs через `voryn_pipeline_block_jobs{status="failed"}`.
